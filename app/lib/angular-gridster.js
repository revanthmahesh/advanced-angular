...
(function(root, factory) {
	...
	.constant('gridsterConfig', {
		columns: 48, // number of columns in the grid
		...
		rowHeight: '15', // height of grid rows. 'match' will make it the same as the column width, a numeric value will be interpreted as pixels, '/2' is half the column width, '*5' is five times the column width, etc.
				...
				compile: function($tplElem) {
					...
					return function(scope, $elem, attrs, gridster) {
						...
						function updateHeight() {
							...
							/* setTimeout(function(){
							$elem.css('min-height', $(document).height() + 'px');
							}, 1000); */
						}
		...
		function($document, $window, GridsterTouch) {
			function GridsterDraggable($el, scope, gridster, item, itemOptions) {
				...
				function dragStop(event) {
					...
					var _width = $('.cou-drop-area').width();
					var _elmWw = ((elmW/_width)*100).toFixed(0);
					item.top=elmY;
					item.left=elmX;
					item.height=elmH;
					item.width=_elmWw;
					...
					scope.$emit('itemChange', item);
				}

				function mouseDown(e) {
					if (document.querySelector('.gridster-item-moving') !== null)return;
		...
		function GridsterResizable($el, scope, gridster, item, itemOptions) {
			...
			function ResizeHandle(handleClass) {
				...
				function mouseMove(e) {
					...
					var elmWw = ((elmW/width)*100).toFixed(0);
					// set new position
					$el.css({
						'top': elmY + 'px',
						'left': elmX + 'px',
